...
import com.lasat.server.exceptions.IncorrectValueException;
...
public class ConsoleController {
    ...
    public Person createPerson() {
        ...
        try {
            person.setId(Person.generateUniqueId(dataController.getSortedMap()));
            ...
            input = scanner.nextLine();
            try {
                person.setHeight(Long.parseLong(input));
            } catch (NumberFormatException e) {
                throw new IncorrectValueException("значение поля роста человека должно быть целочисленным");
            }

            System.out.print("Введите положительное значение веса человека: ");
            input = scanner.nextLine();
            try {
                person.setWeight(Float.parseFloat(input));
            } catch (NumberFormatException e) {
                System.out.println("Значение веса должно быть числом с плавающей точкой. Поле было пропущено.");
            } catch (IncorrectValueException e) {
                System.out.println("Вес должен быть больше 0. Поле было пропущено.");
            }
            ...
            input = scanner.nextLine();
            try {
                person.getLocation().setY(Double.parseDouble(input));
            } catch (NumberFormatException e) {
            ...
            input = scanner.nextLine();
            try {
                person.getLocation().setZ(Integer.parseInt(input));
            } catch (NumberFormatException e) {
    ...
    public Person updatePerson(final Person oldPerson) {
        ...
        try {
            System.out.println("Введите положительный вес человека");
            input = scanner.nextLine();
            if (!input.equals(""))
                person.setWeight(Float.parseFloat(input));
            else System.out.println("Поле не было изменено");
        } catch (IncorrectValueException | NumberFormatException e) {
            System.out.println("Некорректно введён вес человека, поле осталось прежним");
        }
        try {
