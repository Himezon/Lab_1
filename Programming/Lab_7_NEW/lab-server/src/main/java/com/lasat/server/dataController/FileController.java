...
public class FileController {
    ...
    public String readCSVFile (final String path) {
        if (path.equals("")) {
            return path;
        }
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(path)))) {
            ...
            for (int i=0;i<persons.size();i++) {
                try {
                    ...
                    return "Ошибка: " + e.getMessage()+". Человек по строке "+(i+1)+" не был создан.";
                }
                return "Считывание прошло успешно.";
            }
        } catch (IOException e) {
            return "Ошибка чтения файла: " + path;
        }
        return "Коллекция была успешно считана из файла.";
    }

    public String writeCSVFile (final String path) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(path)))) {
            ...
            for (Long id: dataController.getSortedMap().keySet())
                writer.write(createPersonString(dataController.getSortedMap().get(id))+"\n");
            writer.flush();
        } catch (IOException e) {
            return "Ошибка записи файла " + path;
        }
    ...
    private Person parsePerson (final String personStr) throws IncorrectValueException {
        ...
        try {
            if (Person.checkUniqueId(dataController.getSortedMap(), Long.parseLong(args[0])))
